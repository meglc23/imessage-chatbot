...
class iMessageHandler:
        ...
        try:
            ...
            for row in reversed(cursor.fetchall()):  # Reverse to get chronological order
                ...
                if is_reaction:
                    ...
                    # Regular message
                    if text is None:
                        text = self._decode_attributed_body(attributed_body)
    ...
    def _decode_attributed_body(self, blob: Optional[bytes]) -> Optional[str]:
        """Attempt to extract plain text from an attributedBody blob."""
        if not blob:
            return None
        ...
        try:
            ...
            if preferred:
                cleaned = preferred
            if cleaned:
                return max(cleaned, key=lambda seg: (score(seg), len(seg)))
        except Exception:
            pass
